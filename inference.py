...
class CFG:
    ...
    comp_dataset_path = f'/content/gdrive/MyDrive/modelmain/inference-in/'
    ...
    outputs_path = f'/content/gdrive/MyDrive/modelmain/inference-out/{comp_name}/{exp_name}/'
...
# Use regex to extract the part that includes 'epoch=XX.ckpt'
pattern = r'epoch=\d+\.ckpt'
match = re.search(pattern, args.model_path)
if match:
    model_info = match.group()
else:
    model_info = 'unknown_model'

# Generate the current timestamp
current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M")

fragments=os.listdir('/content/gdrive/MyDrive/modelmain/inference-in')
...
mask_pred.save(f'{args.out_path}/{fragment_id}_{args.stride}_{args.start_idx}_{model_info}_{current_time}.png')